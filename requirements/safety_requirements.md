...
- **ASIL**: B-inspired
- **Detection Method**: Q&A watchdog timeout on Pi400 Linux supervisor

## Functional Safety Requirements

### FSR-001: Q&A Watchdog Monitoring

**Requirement**: The Pi400 supervisor shall monitor Pi5 liveness and correctness via an UDP challenge/response (Q&A) watchdog.

**Details**:
- Interface: UDP port 9001, dedicated Ethernet 192.168.50.x
- Seed: 32-bit random value generated by Pi400 each cycle
- Response: Pi5 computes `seed XOR 0xA5A5A5A5` and sends via UDP
- Valid window: 30ms total — 15ms closed + 15ms open (response must arrive in open window)
...
**Rationale**: Q&A protocol provides stronger diagnostic coverage than a simple GPIO toggle. A correct response can only be generated by a running, non-corrupted Pi5 process with access to the I2C seed. The failure counter tolerates transient UDP packet loss while detecting persistent failures.

**ASIL**: B-inspired

**Verification**: Fault injection tests:
- Stop UDP response (kill health_node) → verify failure_counter increments → verify SAFE_STATE
- Send wrong answer → verify failure_counter increments
- Send correct answer too early (<15 ms, closed window) → verify failure_counter increments
...
**Details**:
- Supervisor uses only: UDP Q&A seed/response validation, timing window, failure counter, state enum
- No AI inference in Pi400 supervisor domain
...
**Requirement**: The Pi400 Linux supervisor shall run on a physically separate processor from the Pi5 Linux/ROS2 domain.

**Details**:
- Pi 5 hosts Linux/ROS2 (development domain)
- Pi400 (Ubuntu 22.04) hosts the Linux supervisor (safety monitoring domain)
- No shared memory within same address space
- UDP Ethernet + GPIO inter-board communication only
...
- Linux POSIX SCHED_FIFO scheduling for supervisor (Python process)
...
**Details**:
- UDP Q&A messages include CRC-16/CCITT-FALSE and sequence counter
- Pi400 validates CRC-16 and sequence counter before accepting response
- CRC or seq mismatch treated as communication fault → failure_counter++
- Stale response detection via 30ms window timeout

**Rationale**: Detect data corruption due to bit flips or software errors.

**ASIL**: B-inspired (safety mechanism)

**Verification**: FFI-COMM test: ROS2 flood — verified 2026-05-11 (PASS)
...
- Released only after first valid Q&A cycle
...
**Implementation**: Pi400 Linux supervisor directly controls emergency stop GPIO (GPIO 25), independent of Pi5 software
...
|-------------|--------------|----------------|
| FSR-001 | Pi400 UDP Watchdog Server | GPIO monitoring, timeout detection |
| FSR-002 | Pi400 Safe State Controller | Emergency stop GPIO assertion |
| FSR-003 | Pi400 supervisor domain | No AI libraries, rule-based code |
| FSR-004 | System architecture | Dual processor deployment |
| FSR-005 | Pi400 supervisor domain | Local clock, FIFO scheduling |
| FSR-006 | Both domains | CRC-16 generation (Pi5), validation (Pi400) |
| FSR-007 | Pi400 Safe State Controller | Default GPIO state at boot |
...
**Decomposition**:
- **Path 1** (ASIL B): Watchdog supervision on Pi400 Linux supervisor domain
